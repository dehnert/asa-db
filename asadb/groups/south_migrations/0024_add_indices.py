...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        ...
        # Adding index on 'OfficeHolder', fields ['start_time']
        db.create_index('groups_officeholder', ['start_time'])

        # Adding index on 'OfficeHolder', fields ['person']
        db.create_index('groups_officeholder', ['person'])

        # Adding index on 'OfficeHolder', fields ['end_time']
        db.create_index('groups_officeholder', ['end_time'])

        # Adding unique constraint on 'AthenaMoiraAccount', fields ['username']
        db.create_unique('groups_athenamoiraaccount', ['username'])

        # Adding index on 'GroupNote', fields ['author']
        db.create_index('groups_groupnote', ['author'])

        # Adding unique constraint on 'OfficerRole', fields ['slug']
        db.create_unique('groups_officerrole', ['slug'])

        # Adding index on 'Group', fields ['abbreviation']
        db.create_index('groups_group', ['abbreviation'])

        # Adding index on 'Group', fields ['name']
        db.create_index('groups_group', ['name'])

        # Adding unique constraint on 'GroupStartup', fields ['group']
        db.create_unique('groups_groupstartup', ['group_id'])


    def backwards(self, orm):
        
        # Removing unique constraint on 'GroupStartup', fields ['group']
        db.delete_unique('groups_groupstartup', ['group_id'])

        # Removing index on 'Group', fields ['name']
        db.delete_index('groups_group', ['name'])

        # Removing index on 'Group', fields ['abbreviation']
        db.delete_index('groups_group', ['abbreviation'])

        # Removing unique constraint on 'OfficerRole', fields ['slug']
        db.delete_unique('groups_officerrole', ['slug'])

        # Removing index on 'GroupNote', fields ['author']
        db.delete_index('groups_groupnote', ['author'])

        # Removing unique constraint on 'AthenaMoiraAccount', fields ['username']
        db.delete_unique('groups_athenamoiraaccount', ['username'])

        # Removing index on 'OfficeHolder', fields ['end_time']
        db.delete_index('groups_officeholder', ['end_time'])

        # Removing index on 'OfficeHolder', fields ['person']
        db.delete_index('groups_officeholder', ['person'])

        # Removing index on 'OfficeHolder', fields ['start_time']
        db.delete_index('groups_officeholder', ['start_time'])
    ...
    models = {
        ...
        'groups.athenamoiraaccount': {
            ...
            'username': ('django.db.models.fields.CharField', [], {'unique': 'True', 'max_length': '8'})
        },
        'groups.group': {
        ...
        'groups.groupnote': {
            ...
            'author': ('django.db.models.fields.CharField', [], {'max_length': '30', 'db_index': 'True'}),
        ...
        'groups.officeholder': {
            ...
            'end_time': ('django.db.models.fields.DateTimeField', [], {'default': 'datetime.datetime(9999, 12, 31, 23, 59, 59, 999999)', 'db_index': 'True'}),
            'group': ('django.db.models.fields.related.ForeignKey', [], {'to': "orm['groups.Group']"}),
            'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
            'person': ('django.db.models.fields.CharField', [], {'max_length': '30', 'db_index': 'True'}),
