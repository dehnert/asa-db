...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        cycle = orm.GroupConfirmationCycle()
        cycle.name = "Fall 2011"
        cycle.slug = "2011-fall"
        cycle.save()

        confirmations = orm.PersonMembershipUpdate.objects.filter(deleted__isnull=True)
        found_people = {}
        replacements = {}
        for confirmation in confirmations:
            username = confirmation.username
            if username in replacements:
                for group in confirmation.groups.all():
                    replacements[username].groups.add(group)
                confirmation.deleted = datetime.datetime.now()
                confirmation.save()
            elif username in found_people:
                replacement = orm.PersonMembershipUpdate()
                replacements[username] = replacement
                replacement.update_time = datetime.datetime.now()
                replacement.username = username
                replacement.cycle = cycle
                replacement.deleted = None
                replacement.save()
                for group in found_people[username].groups.all():
                    replacement.groups.add(group)
                for group in confirmation.groups.all():
                    replacement.groups.add(group)
                found_people[username].deleted = datetime.datetime.now()
                found_people[username].save()
                del found_people[username]
                confirmation.deleted = datetime.datetime.now()
                confirmation.save()
            else:
                found_people[username] = confirmation
        for username, confirmation in replacements.items():
            confirmation.save()
        
        confirmations = orm.PersonMembershipUpdate.objects.filter(deleted__isnull=True, valid=forms.models.VALID_UNSET)
